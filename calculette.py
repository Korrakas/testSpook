...
class Calculette(Observable):
    ...
    def OpePlus(self):
        self.state = '+'
        self.notify(
            self.state)  # on change letat de loperateur et on le notifie à  tous les observeurs avec la methode notify dans la classe mère
    ...
    def OpeEgal(self):
        ...
        self.calcul_resultat()  # on lance une méthode dans la classe calculette "car c'est elle qui contient le
        # microprocesseur" donc elle peut réaliser des calculs plus complexes
    ...
    def calcul_resultat(self):
        ...
        for i in range(1, len(
                recuperation_journal)):  # bon calcul vite fait pour calculer un truc du style 3 + 4 - 1 * 2 stocké dans le tableau

            contenu_case = recuperation_journal[i]
            if isinstance(contenu_case,
                          str):  # si la case contient un string, donc cest forcément un opérateur, on fait +=, -= etc avec le contenu de la case d'après... normalement un nombre ,(jai pas traité les cas derreurs, osef
        ...
        self.notify(
            mon_calcul)  # et on crie le résultat du calcul aux observers (notify revient à la classe mère Observable qui utilise la méthode update d' observers..)
...
class Journal(Observer):
    ...
    def update(self, datas):
        ...
        for n in self.journal:
            print(n,
                  end='')  # mise en forme du print, normalement la valeur par défaut de end est \n pour aller à la ligne après chaque print,
